...
class Solver(object):
    def __init__(self, train_loader, test_loader, config, save_fold=None):
        ...
        if self.config.pre_trained: 
            ...
            pre_model=torch.load(self.config.pre_trained)
            net_dict=self.net_bone.state_dict()
            state_dict={k: v for k,v in pre_model.items() if k in net_dict.keys()}
            net_dict.update(state_dict)
            self.net_bone.load_state_dict(net_dict)
            ...
            for i, data_batch in enumerate(self.train_loader):
                ...
                for ix in up_sal:
                    sal_loss1.append(F.binary_cross_entropy_with_logits(ix, sal_label, reduction='sum')
                                    +1-ssim_loss(ix,sal_label)+iou_loss(ix,sal_label))
